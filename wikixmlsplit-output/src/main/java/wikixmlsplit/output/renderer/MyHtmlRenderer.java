...
import org.sweble.wikitext.engine.output.*;
...
public class MyHtmlRenderer extends HtmlRendererBase implements CompleteEngineVisitorNoReturn {
	@Override
	...
	public void visit(WtImageLink n) {
		...
		MediaInfo info;
		try {
			info = callback.getMediaInfo(target.getNormalizedFullTitle(), imgWidth, imgHeight);
		} catch (Exception e) {
			throw new VisitingException(e);
		}
		...
		boolean hasThumbFrame = isImage && n.getFormat() == ImageViewFormat.THUMBNAIL
		...
		if (linkTarget != null || linkUrl != null) {
			pf("<a href=\"%s\"%s%s>", linkTarget != null ? callback.makeUrl(linkTarget) : callback.makeUrl(linkUrl),
					aClasses, aTitle);
		}
		...
		if (exists) {
			if (isImage) {
				pt("<img alt=\"%s\" src=\"%s\" width=\"%d\" height=\"%d\"%s />", alt.trim(), imgUrl, width, height,
						imgClasses);
			} else {
		...
		if (!callback.resourceExists(target)) {
			...
			if (n.hasTitle()) {
				pt("<a href=\"%s\" class=\"new\" title=\"%s (page does not exist)\">%=%!%=</a>",
						callback.makeUrlMissingTarget(path), title, n.getPrefix(), n.getTitle(), n.getPostfix());
			} else {
				String linkText = makeTitleFromTarget(n, target);

				pt("<a href=\"%s\" class=\"new\" title=\"%s (page does not exist)\">%=%=%=</a>",
						callback.makeUrlMissingTarget(path), title, n.getPrefix(), linkText, n.getPostfix());
			}
		} else {
			if (!target.equals(pageTitle)) {
				if (n.hasTitle()) {
					pt("<a href=\"%s\" title=\"%s\">%=%!%=</a>", callback.makeUrl(target), makeLinkTitle(n, target),
				...
				if (n.hasTitle()) {
					pt("<strong class=\"selflink\">%=%!%=</strong>", n.getPrefix(), n.getTitle(), n.getPostfix());
				} else {
					pt("<strong class=\"selflink\">%=%=%=</strong>", n.getPrefix(), makeTitleFromTarget(n, target),
							n.getPostfix());
				}
			}
		}
	...
	public void visit(WtSection n) {
		p.indent();
		pt("<h%d><span class=\"mw-headline\" id=\"%s\">%!</span></h%d>", n.getLevel(), makeSectionTitle(n.getHeading()),
	...
	public void visit(WtTagExtension n) {
		...
		/*
		 * pc("&lt;%s%!&gt;%=&lt;/%s&gt;", n.getName(), n.getXmlAttributes(),
	...
	/*
	 * private void wrapText(String text) { if (inPre > 0) { p.print(esc(text)); }
	 * else {
	 * 
	 ...
	 * while (i < len) { char ch;
	 * 
	 * // If at beginning of line skip whitespace if (p.atBol()) { while (i < len) {
	 * ch = text.charAt(i); if (!Character.isWhitespace(ch)) break; ++i; } }
	 * 
	 * if (i >= len) break;
	 * 
	 * p.flush(); int col = p.getColumn(); int border = 80 + p.getIndent() * 4;
	 * 
	 * int j = i; while (j < len) { ch = text.charAt(j++); if (col >= border &&
	 * Character.isWhitespace(ch)) break; if (ch == '\n') break; }
	 * 
	 * String substr = text.substring(i, j); if (!substr.isEmpty())
	...
	private String makeSectionTitle(WtHeading n) {
		byte[] title;
		title = makeTitleFromNodes(n).getBytes(StandardCharsets.UTF_8);
	...
	private String makeTitleFromNodes(WtNodeList titleNode) {
		...
		SafeLinkTitlePrinter p = new MySafeLinkTitlePrinter(w, wikiConfig);
		p.go(titleNode);
		return w.toString();
	}
	...
	/**
	 * If the cell content is only one paragraph, the content of the paragraph is
	 * returned. Otherwise the whole cell content is returned. This is done to
	 * render cells with a single paragraph without the paragraph tags.
	 */
	...
	protected WtNodeList cleanAttribs(WtNodeList xmlAttributes) {
		...
		if (style != null)
			newStyle = new StringBuilder(cleanAttribValue(style.getValue()));
		...
		for (WtXmlAttribute a : clean) {
			...
			if (name.equals("align")) {
				newStyle.insert(0, String.format("text-align: %s; ", cleanAttribValue(a.getValue())));
			} else {
				newStyle.insert(0, String.format("%s: %s; ", name, cleanAttribValue(a.getValue())));
			}
		}

		WtXmlAttribute newStyleAttrib = nf.attr(nf.name(nf.list(nf.text("style"))),
				nf.value(nf.list(nf.text(newStyle.toString()))));
	...
	public static <T extends WtNode> String print(MyHtmlRendererCallback callback, WikiConfig wikiConfig,
			PageTitle pageTitle, T node) {
		return print(callback, wikiConfig, new StringWriter(), pageTitle, node).toString();
	}

	public static <T extends WtNode> Writer print(MyHtmlRendererCallback callback, WikiConfig wikiConfig, Writer writer,
			PageTitle pageTitle, T node) {
		new MyHtmlRenderer(callback, wikiConfig, pageTitle, writer).go(node);
		return writer;
	}
	...
	protected MyHtmlRenderer(HtmlRendererCallback callback, WikiConfig wikiConfig, PageTitle pageTitle, Writer w) {
		super(w);
	...
	public void visit(WtExternalLink n) {
		...
		if (n.hasTitle()) {
			...
			pt("<a rel=\"nofollow\" class=\"external text\" href=\"%s\">%!</a>", callback.makeUrl(n.getTarget()),
					n.getTitle());
		} else {
			p.indentAtBol();

			pt("<a rel=\"nofollow\" class=\"external text\" href=\"%s\">%s</a>", callback.makeUrl(n.getTarget()),
